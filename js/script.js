...
function jcalendar_week(options){
	...
	function addDOM(){
		calendarid.html("");
		var before_btn=opts.showbtn?'<button class="switch_month beforem_btn"><img src="img/news/to-left.svg" alt=""></button>':'';
		var after_btn=opts.showbtn?'<button class="switch_month afterm_btn"><img src="img/news/to-right.png" alt=""></button>':'';
		var header_dom = '<div class="flex_i calendar_header ">'+
					before_btn+
					'<div class="flex_auto calendar_info" onclick="$(\'#pop\').show();"></div>'+
					after_btn+
				'</div>';
		var week_dom='<li>星期日</li><li>星期一</li><li>星期二</li><li>星期三</li><li>星期四</li><li>星期五</li><li>星期六</li>';
	...
	var setdaydata = function(year,weeknum){
		...
		for(var i=0;i<7;i++){
			...
			if(newdate.getFullYear()==todayarr[0] && newdate.getMonth()==todayarr[1] && newdate.getDate()==todayarr[2]){
				istoday = '<span class="today_i"></span>';
			}
